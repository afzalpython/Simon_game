<template>
  <div class="game">
    ...
    <aside class="sidebar">
      <section class="play">
        <h1 class="play__round">Уровень: {{ round }}</h1>
        <button class="play__button" @click="init">Старт</button>
        <span v-show="gameover" class="play__result">Извините, вы проиграли после {{ round }} уровня!</span>
      </section>

      <section class="difficulty">
        <h2 class="difficulty__heading">Уровень сложности</h2>
...
<style scoped>
	...
	.gameboard__button:nth-of-type(2) {
		border-radius: 5 250px 0 0;
		border-left-width: 0.5rem;
		border-bottom-width: 0.5rem;
	}

	.gameboard__button:nth-of-type(3) {
		border-radius: 5 0 0 250px;
		border-right-width: 0.5rem;
		border-top-width: 0.5rem;
	}

	.gameboard__button:last-of-type {
		border-radius: 5 0 250px 0;
		border-top-width: 0.5rem;
		border-left-width: 0.5rem;
	}
